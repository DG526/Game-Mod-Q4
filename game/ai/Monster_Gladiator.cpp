...
void rvMonsterGladiator::Think(void) {
	if (!CheckDormant()) {
		UpdateAnimation();
		Present();
	}
	...
	if (gameLocal.time >= roundStartTime + 3333 && chosenMove != 'n') {
		rvMonsterGladiator::Winner winner = GetWinner();
		if (winner == Monster)
...
char rvMonsterGladiator::GetMove(void) {
	idRandom2 rand;
	rand.SetSeed(gameLocal.time % 69420);
	int choicePool = rand.RandomInt(8);
...
char rvMonsterGladiator::Cheat(void) {
...
rvMonsterGladiator::Winner rvMonsterGladiator::GetWinner(void) {
...
void rvMonsterGladiator::ForceKill(void) {
	player->inMatch = false;
	player->opponent = idPlayer::OP_NONE;
	AdjustHealthByDamage(health);
	Killed(player, player, health, vec3_zero, INVALID_JOINT);
...
void rvMonsterGladiator::LootDrop(void) {
	idRandom2 rand;
	rand.SetSeed(gameLocal.time % 69420);
