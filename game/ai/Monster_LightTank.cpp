...
void rvMonsterLightTank::Think(void) {
	if (!CheckDormant()) {
		UpdateAnimation();
		Present();
	}
	...
	if (gameLocal.time >= roundStartTime + 3333 && chosenMove != 'n') {
		rvMonsterLightTank::Winner winner = GetWinner();
		if (winner == Monster)
...
char rvMonsterLightTank::GetMove(void) {
	idRandom2 rand;
	rand.SetSeed(gameLocal.time % 69420);
	int choicePool = rand.RandomInt(5);
...
rvMonsterLightTank::Winner rvMonsterLightTank::GetWinner(void) {
...
void rvMonsterLightTank::ForceKill(void) {
	player->inMatch = false;
	player->opponent = idPlayer::OP_NONE;
	AdjustHealthByDamage(health);
	Killed(player, player, health, vec3_zero, INVALID_JOINT);
...
void rvMonsterLightTank::LootDrop(void) {
	idRandom2 rand;
	rand.SetSeed(gameLocal.time % 69420);
